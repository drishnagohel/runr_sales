...
class Creator extends Model
{
    ...
    public static function getcreator($data)
    {

        $query = DB::table('tbl_creator as c')->select('c.*', 'u.name as createdby_fname', 'u.last_name as createdby_lname', 'us.name as updatedby_fname', 'us.last_name as updatedby_lname')
            ->leftjoin('tbl_user as u', 'c.created_by', '=', 'u.user_id')
            ->leftjoin('tbl_user as us', 'c.updated_by', '=', 'us.user_id');

        if (array_key_exists('creator_id', $data) && isset($data['creator_id'])) {
